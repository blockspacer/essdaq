...
-- graycode to integer conversion (for BCID field)
...
function srsvmm_proto.dissector(buffer,pinfo,tree)
	...
	if dataid == 0x564d33 then
	  	if protolen >= 16 then
			...
			if (fc-fc0) > 0 then
				if last_fc > fc and (last_fc-fc) > 0xFFFF0000  then
					pinfo.cols.protocol = "FC OVERLFOW"
				end
				if  fc - last_fc == 0 then
					pinfo.cols.protocol = "FC DUPLICATE"
				end
				if fc - last_fc > 1 then
					pinfo.cols.protocol = "FC MISSING"
				end
			end
			...
			local hit_id = 0
			local marker_id = 0
			hits = (protolen-16)/data_length_byte
			for i=1,hits do
				...
				if flag == 0 then
					...
					if last_timestamp1 > timestamp1 or (last_timestamp1 == ltimestamp1 and last_timestamp2 > timestamp2) then
						...
						pinfo.cols.protocol = "TIME ERROR"
					end
			...
			pinfo.cols.info = string.format("FEC: %d, Hits: %4d, Markers: %4d, Time error: %4d", fecid, hit_id, marker_id, time_error)
	  	end
